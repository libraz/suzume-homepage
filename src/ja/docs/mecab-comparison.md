...
このページの MeCab の境界と表示 feature は、すべて **MeCab 0.996 と
mecab-ipadic 2.7.0-20070801**（UTF-8 版 IPA 辞書）の出力から記録しています。
ラベルは読みやすいように MeCab のカンマ区切り feature を略記していますが、
トークン境界は書き換えていません。`mecab` をデフォルト設定で実行し、
ユーザー辞書は読み込んでいません。使用中の設定は次のコマンドで確認できます。
...
MeCab の出力は、選択した辞書、そのコスト、ユーザー辞書によって変わります。
UniDic、NEologd、カスタマイズした IPA 辞書では、このページと異なる境界や品詞に
なることがあります。上記の正規化パイプラインは、この IPA 辞書の出力を起点に、
Suzume の一般化した比較規則を適用してテスト期待値を生成します。
...
|--|-------|--------|
| **アプローチ** | 辞書駆動型 | 特徴量駆動型 |
| **辞書** | 外部辞書が必要。サイズと出力は選択した辞書に依存 | コンパクトな辞書を同梱（WASM込みでgzip約<WasmSize />） |
...
採用した IPA 辞書では「と共に」や、文脈によっては「につれて」も最初から
1トークンになるため、この比較条件では境界差として掲載しません。別の辞書では
分割される場合があります。
...
ASCII の単語文字に挟まれたアポストロフィ、アンパサンド、スラッシュも同じ規則で
1トークンに保ちます。
...
「をもって」も同じ規則で「を / もっ / て」になります。生産的な動詞境界を
含まない閉じた表現は、上の「や否や」のように全体を保ちます。
...
否定構文の「読みっこない」も同じ方針で、読み（`VERB`）/ っこ（`SUFFIX`）/
ない（`ADJ`）に分かれます。
...
文語の活用も現代語と同じ方針で文法境界を保ちます。否定、推量・当為、過去・完了、
禁止、命令の各系列を扱います。例を挙げます。
...
名詞述語の後では、「じゃ」をコピュラ、「ない」を否定の助動詞として扱います。
単独の「じゃない」は曖昧で、1つの形容詞トークンになる場合があるため、比較には
名詞側も含めます。
...
Suzume は、未知の同一文字種複合語の内部にある任意の語彙境界を推測できません。
ただし、文法規則やコンパクトな辞書で根拠のある境界は分割できます。
...
**回避策:** アプリケーションで必要な境界は、[ユーザー辞書ガイド](/ja/docs/user-dictionary)の実行時読み込み例を使って登録します。JavaScript、Python、Go、C++、C、ネイティブ CLI の例を掲載しています。
