...
|--|-------|--------|
| **アプローチ** | 辞書駆動型 | 特徴量駆動型 |
| **辞書** | 50MB以上（必須） | 最小限（WASM含めgzipで約435KB） |
...
**理由:** 多くのアプリケーション（検索、タグ付け、表示）では、「3人」や「三ヶ月間」を1つの単位として扱い、前後の名詞から分離する方が有用です。
...
**理由:** 通常の名前は敬称を分離する方が検索や表示に有用ですが、短いひらがな愛称は全体が検索対象になります。
...
**理由:** 長音は修飾する語の一部です。連続する長音は表層形に保持し、原形では通常形に正規化します。
...
**理由:** 「ずに」は「〜しないで」を表すまとまった検索単位として機能します。MeCab がすでに一語で出力する「かも」「のに」は差異例から除外しています。
...
**理由:** 「〜方」は動作方法を表す語彙的な検索単位であり、動詞語幹と一緒に扱う方が検索に適しています。
...
**理由:** 「た」と「がる」はこの構文では過去＋動詞ではなく、一体で希望を表す助動詞です。
...
**理由:** 「か」は代名詞の一部ではなく不定を作る助詞であり、「いる」はこの文脈では進行の補助動詞ではありません。
