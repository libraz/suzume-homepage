...
Suzume はネイティブの C / C++ プログラムへ直接リンクできます。WebAssembly も JavaScript も、実行時依存もありません。コアは WASM・Python の各バインディングが利用するものと同じ C++ トークナイザーで、ここではそれを通常の静的／共有ライブラリとして利用したり、辞書を埋め込んで組み込んだりします。
...
`analyze()` は `std::vector<suzume::Morpheme>`（C++）、または `suzume_morpheme_t` の配列を保持する `suzume_result_t`（C）を返します。コンパクトな C ABI は安定した数値コードとビットフィールドを使い、C++ ラッパーが所有権を持つ文字列と bool へデコードします。

| C++（`Morpheme`） | C（`suzume_morpheme_t`） | 説明 |
|-------------------|--------------------------|------|
| `surface` | `surface` | テキスト中に現れる表層形 |
| `pos` | `pos` | C++ では英語 POS ラベル、C では安定した `SUZUME_POS_*` 数値コード |
| `lemma` | `base_form` | 辞書の原形（レンマ） |
| `pos_ja` | `pos` から導出 | 品詞の日本語名 |
| `conj_type` | `conjugation_type` | C++ ではラベル、C では安定した数値コード（`0` は該当なし） |
| `conj_form` | `conjugation_form` | C++ ではラベル、C では安定した数値コード |
| `extended_pos` | `extended_pos` | C++ ではラベル（例: `VERB_連用`）、C では安定した数値コード |
| `start` | `start` | 正規化後テキストでの開始文字オフセット |
| `end` | `end` | 正規化後テキストでの終了文字オフセット |
| `is_user_dict` | `flags & SUZUME_MORPHEME_USER_DICT` | 読み込んだユーザー辞書由来のエントリ |
| `is_formal_noun` | `flags & SUZUME_MORPHEME_FORMAL_NOUN` | 形式名詞のエントリ |
| `is_low_info` | `flags & SUZUME_MORPHEME_LOW_INFO` | 情報量の低い語 |
| `is_unknown` | `flags & SUZUME_MORPHEME_UNKNOWN` | 未知語 |
| `is_from_dictionary` | `flags & SUZUME_MORPHEME_FROM_DICTIONARY` | 辞書からマッチしたエントリ |
| `score` | `score` | 形態素の解析スコア |

C ヘッダーは英語・日本語のラベル文字列を重複して保持しません。`SUZUME_POS_*` 定数をそのまま使うか、上の C 例のようにアプリケーション境界で変換してください。JavaScript・Python・C++ ラッパーには安定したラベル表が同梱されています。
